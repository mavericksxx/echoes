...
/** Whether `ip` may trigger another Gemini call right now, given today's
 * usage_log rows (endpoint LIKE 'gemini:%'). Checked *before* calling
 * Gemini; callers that get `false` back must serve cached/fallback results
 * instead (see genre-resolution.ts) and say so in the response. */
export async function geminiQuotaAvailable(env: Env, ip: string): Promise<boolean> {
  ...
  const globalRow = await env.DB.prepare(
    ...
    .bind(since)
    .first<{ n: number }>();
  if ((globalRow?.n ?? 0) >= GEMINI_DAILY_GLOBAL_CAP) return false;
 ...
 * Phase 7a classify-moods, or Phase 7c caption-generation request, regardless of how many items were in
 * it) against both the global and per-IP daily caps. */
export async function logGeminiCall(env: Env, ip: string, kind: "genres" | "artists" | "moods" | "captions"): Promise<void> {
  try {
